...
String DeviceConfigService::buildPage() const {
  ...
  page += F("<section class='card'><h2>Datove zdroje</h2><label for='adsb_url'>Lokalni ADS-B URL (aircraft.json)</label><input id='adsb_url' name='adsb_url' maxlength='180' required value='");
  ...
  page += F("'></div><div><label for='wu_key'>Novy WU API klic</label><input id='wu_key' name='wu_key' type='password' maxlength='96' placeholder='Prazdne = zachovat ulozeny klic'></div></div><p class='muted'>Lokalni prijimac ma prioritu. Pro letadla mimo jeho dosah se automaticky doplnuji ADS-B/MLAT data primarne z <a href='https://adsb.fi/' target='_blank' rel='noopener'>adsb.fi</a>; pri chybe se pouzije fallback adsb.lol pro oblast cele CR. Predpoved Open-Meteo funguje i bez WU klice. Na displeji se zobrazuji pouze horizonty +3, +6 a +9 hodin.</p></section>");
