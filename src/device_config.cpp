...
void DeviceConfigService::serviceNetwork() {
  ...
  if (stationConnected()) {
    if (asyncReconnectActive_ || asyncReconnectProfile_ >= 0) {
      ...
      if (connectedProfile >= 0) {
        ...
        nextWifiProfileIndex_ = static_cast<size_t>(connectedProfile);
      }
  ...
  if (!asyncReconnectActive_) {
    if (asyncReconnectNextCycleMs_ != 0 &&
        static_cast<int32_t>(asyncReconnectNextCycleMs_ - now) > 0) {
      return;
    }
    WiFi.mode(portalActive_ ? WIFI_AP_STA : WIFI_STA);
    WiFi.setSleep(false);
    asyncReconnectActive_ = true;
    asyncReconnectVisited_ = 0;
  ...
  if (asyncReconnectProfile_ >= 0) {
    const wl_status_t status = WiFi.status();
    const uint32_t elapsed = now - asyncReconnectAttemptStartedMs_;
    const bool definiteFailure =
        elapsed > 600U && (status == WL_NO_SSID_AVAIL ||
                           status == WL_CONNECT_FAILED);
    if (!definiteFailure && elapsed < 4500U) return;

    DebugLog::printf("WiFi async: profile %d failed, status=%d after %u ms\n",
                     static_cast<int>(asyncReconnectProfile_ + 1),
                     static_cast<int>(status), static_cast<unsigned>(elapsed));
    WiFi.disconnect(false, false);
    asyncReconnectProfile_ = -1;
  }

  while (asyncReconnectVisited_ < WIFI_PROFILE_COUNT) {
    const size_t profileIndex = asyncReconnectIndex_;
    asyncReconnectIndex_ = (asyncReconnectIndex_ + 1U) % WIFI_PROFILE_COUNT;
    ++asyncReconnectVisited_;

    const WifiProfile& profile = settings_.wifiProfiles[profileIndex];
    if (!profile.enabled || profile.ssid.isEmpty()) continue;

    DebugLog::printf("WiFi async: trying profile %u/%u SSID '%s'\n",
                     static_cast<unsigned>(profileIndex + 1U),
                     static_cast<unsigned>(WIFI_PROFILE_COUNT),
                     profile.ssid.c_str());
    WiFi.begin(profile.ssid.c_str(), profile.password.c_str());
  ...
  // Full profile cycle failed. Keep/bring up the configuration AP and retry
  // later without blocking the UI task in the meantime.
  asyncReconnectActive_ = false;
  asyncReconnectProfile_ = -1;
  activeWifiProfile_ = -1;
  ...
  DebugLog::printf("WiFi async: all profiles failed; retry in %u s\n",
...
void DeviceConfigService::startWebServer() {
  ...
  server_.on("/adsb-refresh", HTTP_POST,
             [this]() { handleAdsbInternetRefresh(); });
...
bool DeviceConfigService::consumeAdsbInternetRefreshRequested() {
  const bool requested = adsbInternetRefreshRequested_;
  adsbInternetRefreshRequested_ = false;
  return requested;
}
...
String DeviceConfigService::buildDiagnosticsPage() const {
  ...
  page += F("<section class='card'><h2>Sitovy worker</h2><table><tr><td>Task / stav</td><td id='net_worker_state'>--</td></tr><tr><td>Aktivni uloha</td><td id='net_worker_job' class='value'>--</td></tr><tr><td>Fronta</td><td id='net_worker_pending'>--</td></tr><tr><td>Posledni uloha</td><td id='net_worker_last' class='value'>--</td></tr><tr><td>Posledni / nejdelsi</td><td id='net_worker_duration'>--</td></tr><tr><td>OK / chyby / backoff</td><td id='net_worker_counts'>--</td></tr><tr><td>TLS guard</td><td id='tls_guard_state'>--</td></tr><tr><td>TLS interni heap / blok</td><td id='tls_guard_heap'>--</td></tr><tr><td>Odlozeno / vynuceno / WSS yield</td><td id='tls_guard_counts'>--</td></tr></table></section>");
  ...
  page += F("<section class='card wide'><h2>Datove zdroje</h2><table><tr><td>Radar</td><td id='radar_status' class='value'>--</td></tr><tr><td>Radarove snimky</td><td id='radar_frames'>--</td></tr><tr><td>Stari aktualizace radaru</td><td id='radar_age'>--</td></tr><tr><td>Blesky LightningMaps</td><td id='lightning_status' class='value'>--</td></tr><tr><td>Blesky v bufferu</td><td id='lightning_strikes'>--</td></tr><tr><td>Stari aktualizace blesku</td><td id='lightning_age'>--</td></tr><tr><td>ADS-B</td><td id='adsb_status' class='value'>--</td></tr><tr><td>Pocet letounu</td><td id='aircraft_count'>--</td></tr><tr><td>Lokalni / sit / MLAT</td><td id='aircraft_sources'>--</td></tr><tr><td>Stari ADS-B dat</td><td id='adsb_age'>--</td></tr><tr><td>ADS-B lokalni</td><td id='adsb_local_diag' class='value'>--</td></tr><tr><td>ADS-B internet</td><td id='adsb_internet_diag' class='value'>--</td></tr><tr><td>HTTP / posledni pokus</td><td id='adsb_internet_http'>--</td></tr><tr><td>Chyby / dalsi pokus</td><td id='adsb_internet_retry'>--</td></tr><tr><td>Posledni dobry internet snapshot</td><td id='adsb_internet_last_count'>--</td></tr><tr><td>Rucni obnova</td><td><form method='post' action='/adsb-refresh' style='display:inline'><button type='submit'>Obnovit internetove ADS-B</button></form></td></tr><tr><td>Aktualni pocasi</td><td id='weather_status' class='value'>--</td></tr><tr><td>Stari pocasi</td><td id='weather_age'>--</td></tr><tr><td>Predpoved</td><td id='forecast_status' class='value'>--</td></tr><tr><td>Stari predpovedi</td><td id='forecast_age'>--</td></tr><tr><td>Astronomie</td><td id='astronomy_status' class='value'>--</td></tr><tr><td>Stari astronomie</td><td id='astronomy_age'>--</td></tr></table></section>");
...
void DeviceConfigService::handleDiagnosticsJson() {
  ...
  json += F(",\"lcd_load_guard_triggers\":");
  json += String(diagnostics.lcdLoadGuardTriggerCount);
  json += F(",\"longest_loop_ms\":");
  json += String(diagnostics.longestLoopDurationMs);
  json += F(",\"network_worker_alive\":");
  json += boolJson(diagnostics.networkWorkerAlive);
  json += F(",\"network_worker_running\":");
  json += boolJson(diagnostics.networkWorkerRunning);
  json += F(",\"network_worker_paused\":");
  json += boolJson(diagnostics.networkWorkerPaused);
  json += F(",\"network_pending_jobs\":");
  json += String(diagnostics.networkPendingJobs);
  json += F(",\"network_last_job_ms\":");
  json += String(diagnostics.networkLastJobDurationMs);
  json += F(",\"network_longest_job_ms\":");
  json += String(diagnostics.networkLongestJobDurationMs);
  json += F(",\"network_completed_jobs\":");
  json += String(diagnostics.networkCompletedJobs);
  json += F(",\"network_failed_jobs\":");
  json += String(diagnostics.networkFailedJobs);
  json += F(",\"network_backoff_skips\":");
  json += String(diagnostics.networkBackoffSkips);
  json += F(",\"tls_guard_low_memory\":");
  json += boolJson(diagnostics.tlsGuardLowMemory);
  json += F(",\"tls_memory_state\":");
  json += String(diagnostics.tlsMemoryState);
  json += F(",\"tls_free_internal\":");
  json += String(diagnostics.tlsFreeInternal);
  json += F(",\"tls_largest_internal_block\":");
  json += String(diagnostics.tlsLargestInternalBlock);
  json += F(",\"tls_deferred_jobs\":");
  json += String(diagnostics.tlsDeferredJobs);
  json += F(",\"tls_forced_attempts\":");
  json += String(diagnostics.tlsForcedAttempts);
  json += F(",\"tls_recovery_requests\":");
  json += String(diagnostics.tlsRecoveryRequests);
  json += F(",\"tls_consecutive_defers\":");
  json += String(diagnostics.tlsConsecutiveDefers);
  json += F(",\"network_active_job\":\"");
  json += jsonEscape(diagnostics.networkActiveJob);
  json += F("\",\"network_last_result\":\"");
  json += jsonEscape(diagnostics.networkLastResult);
  json += F("\"");
  json += F(",\"adsb_internet_failures\":");
  json += String(diagnostics.adsbInternetFailures);
  json += F(",\"adsb_internet_attempt_age_ms\":");
  json += String(ageMs(now, diagnostics.adsbInternetLastAttemptMs));
  json += F(",\"adsb_internet_success_age_ms\":");
  json += String(ageMs(now, diagnostics.adsbInternetLastSuccessMs));
  json += F(",\"adsb_internet_last_ms\":");
  json += String(diagnostics.adsbInternetLastDurationMs);
  json += F(",\"adsb_internet_retry_ms\":");
  json += String(diagnostics.adsbInternetNextRetryMs);
  json += F(",\"adsb_internet_http_code\":");
  json += String(diagnostics.adsbInternetHttpCode);
  json += F(",\"adsb_internet_last_aircraft\":");
  json += String(static_cast<unsigned>(diagnostics.adsbInternetLastAircraftCount));
  json += F(",\"adsb_internet_last_mlat\":");
  json += String(static_cast<unsigned>(diagnostics.adsbInternetLastMlatCount));
  json += F(",\"adsb_internet_source\":\"");
  json += jsonEscape(diagnostics.adsbInternetSource);
  json += F("\",\"adsb_internet_result\":\"");
  json += jsonEscape(diagnostics.adsbInternetResult);
  json += F("\",\"adsb_local_failures\":");
  json += String(diagnostics.adsbLocalFailures);
  json += F(",\"adsb_local_success_age_ms\":");
  json += String(ageMs(now, diagnostics.adsbLocalLastSuccessMs));
  json += F(",\"adsb_local_result\":\"");
  json += jsonEscape(diagnostics.adsbLocalResult);
  json += F("\"");
...
void DeviceConfigService::handleAdsbInternetRefresh() {
  adsbInternetRefreshRequested_ = true;
  DebugLog::println("Config: manual adsb.fi refresh requested");
  server_.sendHeader("Location", "/diagnostics", true);
  server_.send(303, "text/plain", "Internet ADS-B refresh scheduled");
}
